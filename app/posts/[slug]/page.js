    ...
    <div className='py-[70px] flex flex-col xl:flex-row justify-center gap-7'>
      <div className='post w-full xl:max-w-[770px] flex flex-col gap-12'>
        {post?.img && (
          <div className='rounded-lg'>
            <Image src={post.img} priority={true} alt='' width={400} height={400} className='min-h-full w-full object-cover rounded-lg' /> {/* Display post image */}
          </div>
        )}
        <h1 className='text-2xl sm:text-4xl lg:text-[42px] font-bold'>{post?.title}</h1> {/* Post title */}
        {post?.user && (
          <div className="flex flex-wrap gap-2 items-center sm:gap-4">
            <div className='text-gray-500 flex flex-wrap gap-2 sm:gap-4 items-center'>
              ...
              <div className='flex gap-2 items-center'>
                <div>
                  <span className="text-sm sm:text-base capitalize">{post?.user?.name}</span> {/* Author name */}
                </div>
                <div className="text-sm sm:text-base">|</div>
                <div>
                  <span className="text-sm sm:text-base">{post?.createdAt}</span> {/* Post creation date */}
                </div>
              </div>
            </div>
            <div>
              <span className="capitalize rounded-full px-3 py-1 border bg-teal-400/50">{post?.catSlug}</span> {/* Post category */}
            </div>
          </div>
        )}
        <div className='post-desc flex gap-6 flex-col'>
          {post.desc ? parse(post.desc) : 'Description is not available'} {/* Parse and display post description */}
        </div>
        <div className='flex justify-between items-center'>
          ...
          <div className='flex gap-2 items-center'>
            ...
            <Socialicons className='flex gap-2 items-center' iconClass='text-2xl' /> {/* Social media icons */}
          </div>
        </div>
        {post?.user && (
          <div className='flex flex-wrap xl:flex-nowrap gap-8 items-center'>
            <div className='max-w-32 w-full min-h-32 flex items-center'>
              <Link href={`/authors/${post.user.id}`}>
                <Image src={post?.user?.image || noavatar} alt='' className='rounded-full object-cover' width={120} height={120} /> {/* Link to author's profile */}
              </Link>
            </div>
            <div className='max-w-[617px] w-full flex flex-col gap-2'>
              <h2 className='font-medium text-xl xl:text-2xl capitalize'>
                Author: <Link href={`/authors/${post.user.id}`}>{post.user.name}</Link>
              </h2>
              {post?.user?.bio ? (
                <p>{post.user.bio}</p>
              ) : (
      ...
      <div className='flex flex-col gap-8 justify-start'>
        <div className='side-menu max-w-[370px] rounded-lg border p-4 sm:p-8 lg:p-10 flex flex-col gap-8'>
          <h2 className='capitalize text-2xl font-medium'>Recent Posts</h2> {/* Recent posts section */}
          <div className='flex flex-col gap-8'>
            {Array(3).fill().map((_, index) => (
              <div key={index} className='small-post flex gap-6'>
                <div className='max-w-[70px] w-full min-h-full flex items-center'>
                  <Image src={post2} alt='' width={60} height={60} priority={true} className='rounded-full object-cover w-full min-h-full' /> {/* Placeholder recent post image */}
                </div>
                <div className='flex flex-col gap-2'>
                  <h3 className='font-medium'>Culinary Expeditions: Tasting the ...</h3> {/* Recent post title */}
